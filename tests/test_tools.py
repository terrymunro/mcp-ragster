...
@pytest.mark.asyncio
async def test_load_topic_context(monkeypatch):
    async def run_tasks_sequentially(
        self, topic: str, jina_results: list[dict[str, object]]
    ) -> None:
    ...
    monkeypatch.setattr(
        TopicProcessor, "process_concurrent_tasks", run_tasks_sequentially
    )
...
async def test_query_topic_context():
    ...
    assert response.message is not None and response.message.startswith("Found")
